...
import { sanitizeNumber, sanitizePercentage } from '../lib/utils/sanitizer';
...
export const DemographicsConfigPanel: React.FC<DemographicsConfigProps> = ({
  ...
  return (
    <div className="rounded-lg border bg-white p-4 shadow-sm">
      ...
      <div className={`grid gap-3 ${showGenderSelection ? 'md:grid-cols-2' : 'md:grid-cols-1'}`}>
        <div>
          <label className="text-xs font-medium text-slate-600">Total Records (1-100,000)</label>
          <input
            type="number"
            min={1}
            max={100000}
            value={recordCount}
            onChange={(event) => onRecordCountChange(sanitizeNumber(event.target.value, 1, 100000))}
            className="mt-1 w-full rounded border px-3 py-2 text-sm"
          />
          {recordCount > 50000 && (
            <p className="mt-1 text-xs text-amber-600">
              ⚠️ Large dataset: generation may take 10-30 seconds
            </p>
          )}
        ...
        {showGenderSelection && (
          <div>
            ...
            <input
              ...
              onChange={(event) => {
                const female = sanitizePercentage(event.target.value);
                onChange({
                  ...value,
